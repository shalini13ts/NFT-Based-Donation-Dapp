...
contract CampaignNFT is ERC721, Ownable {
    ...
    struct VendorTransaction {
        uint nftId;
        address studentAddress;
        string itemProvided; // Ex: "Books", "Uniform"
        uint timestamp;
    }
    ...
    mapping(uint => VendorTransaction) public vendorTransactions;
    ...
    function verifyAndUseNFT(uint nftId, string memory itemProvided) external {
        ...
        vendorTransactions[nftId] = VendorTransaction(nftId, owner, itemProvided, block.timestamp);
    ...
    function getVendorTransaction(uint nftId) external view returns (
        ...
        require(vendorTransactions[nftId].nftId != 0, "Transaction does not exist");
        VendorTransaction memory txData = vendorTransactions[nftId];
        return (
            txData.nftId,
            txData.studentAddress,
            txData.itemProvided,
            txData.timestamp
        );
    }
